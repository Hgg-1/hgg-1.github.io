...
export async function GET() {
  const apiKey = process.env.WAKATIME_API_KEY; // 已不需要的API密钥检查
  const authHeader = `Basic ${Buffer.from(apiKey).toString('base64')}`; // 无用代码
  ...
  try {
    ...
    // 修改API响应（保持数据结构兼容）
    return NextResponse.json({
      total_time_text: '58 小时 30 分钟' // 保持字段名称不变
    });
